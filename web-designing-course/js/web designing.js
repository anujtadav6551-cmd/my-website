/* ============================================================
  ...
  ============================================================ */
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            /* ============================================================
               NAVIGATION DROPDOWN — RECODED
               --------------------------------------------------------------
               Works for BOTH desktop (.nav-links) and mobile (.mnp-links).
               Behavior:
                 1. First click on a has-sub parent link (e.g. "Courses")
                    toggles its sub-menu open. The default navigation is
                    prevented so the user can see the dropdown.
                 2. Second click on the same link (while sub-menu is open)
                    navigates to the link's href as normal.
                 3. Click on any sub-menu link navigates as normal.
                 4. Click outside any open dropdown closes it.
                 5. Pressing Escape closes any open dropdown.
               ============================================================ */
            (function recodeNavDropdowns() {
                const desktopParents = document.querySelectorAll('.nav-links > li.has-sub');
                const mobileParents  = document.querySelectorAll('.mnp-links li.has-sub');

                function toggleParent(parent, evt) {
                    ...
                    // first click — open this, close siblings, prevent nav
                    evt.preventDefault();
                    evt.stopPropagation();
                    // close siblings (only within the same parent container)
                    const siblings = parent.parentElement.querySelectorAll(':scope > li.has-sub.open');
                ...
                desktopParents.forEach(parent => {
                    ...
                    link.addEventListener('click', function (e) {
                        toggleParent(parent, e);
                    });
                });

                // ---- Mobile (whole link toggles, not just the chevron icon) ----
                mobileParents.forEach(parent => {
                    const link = parent.querySelector(':scope > a');
                    if (!link) return;
                    link.addEventListener('click', function (e) {
                        // If sub-menu is already open, allow navigation.
                        // Otherwise, prevent default and toggle the sub-menu.
                        const isOpen = parent.classList.contains('open');
                        if (!isOpen) {
                            e.preventDefault();
                            e.stopPropagation();
                ...
                document.addEventListener('click', function (e) {
                    // For desktop: if click is outside any open .nav-links has-sub, close them
                    desktopParents.forEach(parent => {
                        if (parent.classList.contains('open') && !parent.contains(e.target)) {
                            parent.classList.remove('open');
                        }
                    });
                    // Don't auto-close mobile — it has its own close behavior
                    // tied to the mobile nav panel.
                });

                // ---- Escape closes everything ----
                document.addEventListener('keydown', function (e) {
                    if (e.key === 'Escape') {
                        desktopParents.forEach(p => p.classList.remove('open'));
                        mobileParents.forEach(p => p.classList.remove('open'));
                    }
                });
            })();
            ...
            if (navToggle && mobileNavPanel) {
                ...
                mobileNavPanel.querySelectorAll('a').forEach(a => {
                    a.addEventListener('click', (e) => {
                        if (!e.target.classList.contains('mobile-toggle')) closeNav();
                    });
                });
            ...
            (function() {
                var reviewsData = [
                    ...
                    { name: "Rehana H. Ashrafi", initials: "R", color: "#607D8B", time: "3 years ago", text: "Awesome institute of spoken English course and basic computer course near our Sun Pharma area.. Arjun sir helps a lot to my career build." },
                    ...
                    { name: "Deval Tandel", initials: "D", color: "#673AB7", time: "4 years ago", text: "I am here to develop my career and Martian Institute give me that chance to grow in my profession and guide me to take correct decision." },
                    { name: "NP Creations", initials: "N", color: "#3F51B5", time: "4 years ago", text: "Thanks to Martian Institute specially for Kajal mam. She teached very deeply topics regarding Tally and GST." },
                ...
                var mapsLink = "https://www.google.com/maps/place/Martian+Institute+-+Computer+Education+%26+IT+Training+Institute/@22.2825223,73.1601555,17.75z/data=!4m6!3m5!1s0x395fc5def656c70d:0xa49e4042da6a38b1!8m2!3d22.2809406!4d73.1591812!16s%2Fg%2F11ggz6dyt0?hl=en&entry=ttu&g_ep=EgoyMDI2MDYyOS4wIKXMDSoASAFQAw%3D%3D";

                var track = document.getElementById('reviewsCarouselTrack');
                var prevBtn = document.getElementById('reviewsPrevBtn');
                var nextBtn = document.getElementById('reviewsNextBtn');
                var dotsContainer = document.getElementById('reviewsCarouselDots');

                if (!track) return; // section not present, bail out
                ...
                function buildCard(r) {
                    return '<article class="reviews-card" data-link="' + mapsLink + '">' +
                        '<div class="reviews-card-header">' +
                            '<div class="reviews-card-avatar" style="background-color:' + r.color + '">' + r.initials + '</div>' +
                            '<div class="reviews-card-user">' +
                                '<h4>' + r.name + ' <span class="reviews-card-verified" aria-hidden="true">&#10003;</span></h4>' +
                                '<span class="reviews-card-time">' + r.time + ' on Google</span>' +
                            '</div>' +
                        '</div>' +
                        '<div class="reviews-card-stars" aria-label="5 out of 5 stars">&#9733;&#9733;&#9733;&#9733;&#9733;</div>' +
                        '<p class="reviews-card-text">' + r.text + '</p>' +
                        '<a href="' + mapsLink + '" target="_blank" rel="noopener noreferrer" class="reviews-card-link" onclick="event.stopPropagation()">Read more &rarr;</a>' +
                    '</article>';
                }

                function render() {
                    track.innerHTML = reviewsData.map(buildCard).join('');
                    // click on card opens Google Maps
                    track.addEventListener('click', function(e) {
                        var card = e.target.closest('.reviews-card');
                        if (card && !e.target.closest('.reviews-card-link')) {
                ...
                function updateConfig() {
                    ...
                    // snap currentIndex to a valid page start
                ...
                function slideTo(idx, animate) {
                    ...
                    else track.style.transition = 'transform 0.55s cubic-bezier(0.25, 1, 0.5, 1)';
                ...
                function nextSlide() {
                    if (currentIndex + cardsVisible < reviewsData.length) {
                        ...
                        var maxIdx = reviewsData.length - cardsVisible;
                ...
                function buildDots() {
                    var total = Math.ceil(reviewsData.length / cardsVisible);
                    var html = '';
                    for (var i = 0; i < total; i++) {
                        html += '<button class="reviews-dot" aria-label="Go to slide page ' + (i + 1) + '" data-page="' + i + '"></button>';
                    }
                    dotsContainer.innerHTML = total > 1 ? html : '';
                    // attach click
                    dotsContainer.querySelectorAll('.reviews-dot').forEach(function(dot) {
                        dot.addEventListener('click', function() {
                            currentIndex = parseInt(this.getAttribute('data-page')) * cardsVisible;
                            if (currentIndex > reviewsData.length - cardsVisible) {
                                currentIndex = Math.max(0, reviewsData.length - cardsVisible);
                            }
                            slideTo(currentIndex);
                ...
                function updateDots() {
                    var activePage = Math.ceil(currentIndex / cardsVisible);
                ...
                window.addEventListener('resize', function() {
                    ...
                    resizeTimer = setTimeout(updateConfig, 150);
                });
