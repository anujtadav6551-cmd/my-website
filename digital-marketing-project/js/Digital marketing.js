/* ============================================================
  ...
  ============================================================ */
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ---------- NAVIGATION (REWRITTEN — robust dropdown handling) ----------
            // The original code only attached click handlers to .mobile-toggle icons
            // and then closed the panel on ANY <a> click unless e.target itself had
            // .mobile-toggle. But the click target is usually the inner <i> icon, so
            // the test failed and the panel collapsed before the submenu could open.
            // This rewrite uses event delegation + closest() so dropdowns work
            // reliably on both desktop (touch/tap) and mobile.
            ...
            if (navToggle && mobileNavPanel) {
                ...
                // (CSS hover already handles mouse; this adds tap support for touch laptops)
            ...
            // ---------- ACCORDION LOGIC (bulletproof v2) ----------
            // Simplified, flicker-free, fully reliable:
            //   • Click a closed item  → opens it AND closes any other open item (single-open).
            //   • Click an open item   → closes it (toggle off).
            //   • Hover (desktop only) → previews a closed item without locking; mouseleave
            //     closes ONLY items that aren't click-locked. A click-locked item is never
            //     touched by hover, so clicks always feel responsive.
            //   • No close-then-reopen flicker: when clicking a hover-previewed item, we
            //     keep it open and just promote it to "locked".
            document.querySelectorAll('.accordion-item').forEach(item => {
                const h = item.querySelector('.accordion-header');
                if (!h) return;

                // Close every item EXCEPT the one passed in. Clears locks on the ones it closes.
                ...
                h.addEventListener('click', (e) => {
                    ...
                    // Case 1: this item is click-locked open → toggle it closed.
                    if (item._userClicked) {
                        item.classList.remove('active');
                        h.setAttribute('aria-expanded', 'false');
                        item._userClicked = false;
                        return;
                    }

                    // Case 2: this item is currently open via hover-preview (not locked).
                    // Promote it to locked WITHOUT touching its DOM state (no flicker).
                    if (item.classList.contains('active')) {
                        closeAllExcept(item);
                        item._userClicked = true;
                        return;
                    }

                    // Case 3: this item is fully closed → open it, close others, lock it.
                    closeAllExcept(item);
                ...
                // Desktop hover-preview (skipped on touch devices)
                item.addEventListener('mouseenter', () => {
                    if (isMobile()) return;
                    if (item._userClicked) return; // never touch a locked item
                    if (item.classList.contains('active')) return; // already open
                    closeAllExcept(item);
                    item.classList.add('active');
                    h.setAttribute('aria-expanded', 'true');
                });
                ...
                item.addEventListener('mouseleave', () => {
                    if (isMobile()) return;
                    if (item._userClicked) return; // never close a locked item
...
/* ============================================================
   DIGITAL MARKETING v2 — New Component Interactions
   --------------------------------------------------------------
   Adds lightweight enhancements for the new components
   (pillar cards, role cards v2, tools grid, cert roadmap).
   All enhancements are progressive — if JS fails, the page
   still works perfectly via CSS hover states.
   ============================================================ */
(function () {
    'use strict';
    ...
    ready(function () {
        // 1. PILLAR CARDS — toggle "topics expanded" state on click (mobile-friendly)
        ...
        // 2. CERT ROADMAP — activate steps as they scroll into view
        var certSteps = document.querySelectorAll('.cert-step');
        if (certSteps.length && 'IntersectionObserver' in window) {
            ...
            certSteps.forEach(function (step) { io.observe(step); });
        }

        // 3. TOOLS GRID — subtle tilt on hover (desktop only)
        var toolCards = document.querySelectorAll('.tool-card, .tool-card-v3');
        ...
        // 4. ROLE CARDS v2 — reveal salary badge with a pop on scroll
        var roleCards = document.querySelectorAll('.role-card-v2');
        if (roleCards.length && 'IntersectionObserver' in window) {
            var roleIO = new IntersectionObserver(function (entries) {
                entries.forEach(function (entry, i) {
                    if (entry.isIntersecting) {
                        ...
                        roleIO.unobserve(entry.target);
                    }
                });
            }, { threshold: 0.25 });
            roleCards.forEach(function (c) { roleIO.observe(c); });
        }

        // 5. SIDEBAR V2 — animate the trust-row numbers when visible
        var trustNums = document.querySelectorAll('.sidebar-trust-item .trust-num');
        trustNums.forEach(function (el) {
...
/* ============================================================
   DIGITAL MARKETING v3 — Additional interactions
   ============================================================ */
(function () {
    'use strict';
    ...
    ready(function () {
        // ---- "Show More Career Paths" toggle (creative expand button) ----
        var careersToggle = document.getElementById('careersExpandToggle');
        var careersHidden = document.getElementById('careersHiddenPaths');
        var careersGroup = document.getElementById('careersToggleGroup');

        if (careersToggle && careersHidden) {
            ...
            careersToggle.addEventListener('click', function () {
                ...
                if (careersGroup) careersGroup.classList.add('moving');

                setTimeout(function () {
                    if (isExpanded) {
                        // Move the toggle button AFTER the hidden block so it sits below
                        if (careersGroup && careersHidden.parentNode) {
                            careersHidden.parentNode.insertBefore(careersGroup, careersHidden.nextSibling);
                        }
                        careersHidden.classList.add('expanded');
                        careersHidden.setAttribute('aria-hidden', 'false');
                        careersToggle.classList.add('active');
                        careersToggle.setAttribute('aria-expanded', 'true');
                    } else {
                        // Move the toggle button back BEFORE the hidden block
                        if (careersGroup && careersHidden.parentNode) {
                            careersHidden.parentNode.insertBefore(careersGroup, careersHidden);
                        }
                        careersHidden.classList.remove('expanded');
                        careersHidden.setAttribute('aria-hidden', 'true');
                        careersToggle.classList.remove('active');
                        careersToggle.setAttribute('aria-expanded', 'false');
                    }

                    // Pulse ring animation
                    careersToggle.classList.add('pulse-once');
                    setTimeout(function () {
                        careersToggle.classList.remove('pulse-once');
                    }, 700);

                    // Fade-in animation for the moving toggle
                    requestAnimationFrame(function () {
                        if (careersGroup) careersGroup.style.opacity = '0';
                        requestAnimationFrame(function () {
                            if (careersGroup) {
                                careersGroup.classList.remove('moving');
                                careersGroup.style.transition = 'opacity 0.5s ease';
                                careersGroup.style.opacity = '1';
                                setTimeout(function () {
                                    if (careersGroup) careersGroup.style.transition = '';
                                    isAnimating = false;
                                    if (typeof AOS !== 'undefined') AOS.refresh();
                                    if (typeof ScrollTrigger !== 'undefined') ScrollTrigger.refresh();
                                }, 500);
